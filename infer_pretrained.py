...
def main(args):
    ...
    for image_file in os.listdir(args.data_folder):
        ...
        if google_font_mapping.get(predicted):
          print(image_file, *google_font_mapping.get(predicted))
        else:
          print(image_file, "cannot match with google font!")
    
    # Combine arrays into a list of rows
    # combined_arrays = np.row_stack(np_arrays)

    # Save to CSV without using pandas
    with open('probs_ocr_verified.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        
        # Write rows from the combined array
        for row in arrays:
            writer.writerow(row)
